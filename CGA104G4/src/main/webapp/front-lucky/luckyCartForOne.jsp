...
<div class="row">
	...
	<c:forEach var="lucky" items="${list}">
		...
		<div class="col-sm-6 col-md-3 text-center">
	      <img src="<%=request.getContextPath()%>/lucky/lucky.do?action=getImg&luckyId=<%=luckyVO.getLuckyId()%>" width="150px" height="150px">
	      <br>
	      <%=luckyVO.getLkName()%><strong>$<%=luckyVO.getLkPrc()%></strong>
	      <br>
	      <button class="btn btn-danger my-cart-btn" data-id=<%=luckyVO.getLuckyId()%> data-name=<%=luckyVO.getLkName()%> data-summary=<%=luckyVO.getLkCont()%> data-price=<%=luckyVO.getLkPrc()%> data-quantity="1" data-image="<%=request.getContextPath()%>/lucky/lucky.do?action=getImg&luckyId=<%=luckyVO.getLuckyId()%>">我要預約</button>
	      <%=luckyVO.getLkCont()%>
	    </div>	
